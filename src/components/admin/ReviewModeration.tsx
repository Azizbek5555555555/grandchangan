    ...
    <div className="p-6">
      ...
      <div className="mb-4 flex gap-2">
        {["PENDING", "APPROVED", "REJECTED", "all"].map((s) => (
          <button key={s} onClick={() => setFilter(s)} className={`rounded-full px-3 py-1 text-xs ${filter === s ? "bg-brand-red text-white" : "bg-surface-2"}`}>{s}</button>
        ))}
      </div>
      ...
      <div className="space-y-3">
          ...
          <div key={r.id} className="rounded-xl border border-line bg-card p-4">
            <div className="flex items-start justify-between">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <Star key={i} size={14} className={i < r.rating ? "fill-brand-gold text-brand-gold" : "text-muted"} />
                    ))}
                  </div>
                ...
                {r.reply && <p className="mt-2 rounded bg-surface-2 p-2 text-sm"><b>Javob:</b> {r.reply}</p>}
              </div>
            ...
            {replyId === r.id && (
              <div className="mt-3 flex gap-2">
                <input value={replyText} onChange={(e) => setReplyText(e.target.value)} placeholder="Javob yozing..." className="flex-1 rounded-lg border border-line px-3 py-1.5 text-sm" />
        ...
        {shown.length === 0 && <p className="text-muted">Sharh yo'q.</p>}
      </div>
    </div>
  );
}
