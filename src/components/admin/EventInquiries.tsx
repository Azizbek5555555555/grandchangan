  ...
  return (
    <div className="p-6">
      ...
      <div className="space-y-3">
          ...
          <div key={q.id} className="rounded-xl border border-neutral-200 bg-white p-4">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-sm text-neutral-500">{q.phone} · {q.eventType} · {q.guestCount} kishi</p>
                {q.preferredDate && <p className="text-xs text-neutral-400">Sana: {new Date(q.preferredDate).toLocaleDateString("uz-UZ")}</p>}
                {q.message && <p className="mt-1 text-sm text-neutral-600">{q.message}</p>}
              </div>
              <div className="flex items-center gap-2">
                <select value={q.status} onChange={(e) => updateInquiryStatus(q.id, e.target.value)} className="rounded border border-neutral-300 px-2 py-1 text-xs">
        ...
        {inquiries.length === 0 && <p className="text-neutral-400">So'rov yo'q.</p>}
      </div>
    </div>
  );
}
