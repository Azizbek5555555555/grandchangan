      ...
      <div className="overflow-x-auto rounded-xl border border-line bg-card">
        <table className="w-full text-sm">
          ...
          <tbody>
              ...
              <tr key={c.id} className="border-b border-line">
                ...
                <td className="px-4 py-3">
                  <div className="flex gap-1">
                    <button onClick={() => toggleCoupon(c.id, !c.isActive)} className="rounded p-1 hover:bg-surface-2"><Power size={14} /></button>
            ...
            {coupons.length === 0 && <tr><td colSpan={7} className="px-4 py-8 text-center text-muted">Kupon yo'q</td></tr>}
          </tbody>
        </table>
      </div>
        ...
        <Modal open onClose={() => setModal(false)} title="Yangi kupon">
          <div className="space-y-4">
            <label className="block"><span className="mb-1 block text-sm font-medium">Kod</span>
              <input value={code} onChange={(e) => setCode(e.target.value)} placeholder="WELCOME10" className="w-full rounded-lg border border-line px-3 py-2 text-sm uppercase" /></label>
            <label className="block"><span className="mb-1 block text-sm font-medium">Turi</span>
              <select value={type} onChange={(e) => setType(e.target.value)} className="w-full rounded-lg border border-line px-3 py-2 text-sm">
            ...
            <label className="block"><span className="mb-1 block text-sm font-medium">Qiymat</span>
              <input type="number" value={value} onChange={(e) => setValue(e.target.value)} className="w-full rounded-lg border border-line px-3 py-2 text-sm" /></label>
            <label className="block"><span className="mb-1 block text-sm font-medium">Min buyurtma (ixtiyoriy)</span>
              <input type="number" value={minOrder} onChange={(e) => setMinOrder(e.target.value)} className="w-full rounded-lg border border-line px-3 py-2 text-sm" /></label>
