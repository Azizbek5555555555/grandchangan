...
export default function SettingsForm({ general, hours }: { general: General; hours: Hour[] }) {
  ...
  return (
    <div className="max-w-2xl p-6">
      ...
      <section className="mb-8 space-y-4 rounded-xl border border-line bg-card p-5">
      ...
      <section className="space-y-2 rounded-xl border border-line bg-card p-5">
        ...
        {h.map((day, idx) => (
          <div key={day.dayOfWeek} className="flex items-center gap-3 text-sm">
            ...
            {!day.isClosed && (
              <>
                <input type="time" value={day.openTime || "11:00"} onChange={(e) => { const n = [...h]; n[idx] = { ...day, openTime: e.target.value }; setH(n); }} className="rounded border border-line px-2 py-1" />
                <span>—</span>
                <input type="time" value={day.closeTime || "23:00"} onChange={(e) => { const n = [...h]; n[idx] = { ...day, closeTime: e.target.value }; setH(n); }} className="rounded border border-line px-2 py-1" />
              </>
            )}
          </div>
        ))}
