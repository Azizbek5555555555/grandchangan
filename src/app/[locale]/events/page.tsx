    ...
    <>
      ...
      <section className="mx-auto max-w-7xl px-6 py-24">
        <div className="mb-14 text-center">
          ...
          <TextReveal text="Har qanday tadbir uchun" className="font-display text-4xl text-content sm:text-5xl" />
        </div>
        <Reveal3D className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4" stagger={0.12}>
          {KINDS.map((k, i) => (
            <div key={i} className="rounded-3xl border border-line bg-card p-7 text-center transition hover:border-brand-gold/40 hover:shadow-xl">
              ...
              <h3 className="font-display text-xl text-content">{k.title}</h3>
              <p className="mt-2 text-sm text-muted">{k.text}</p>
            </div>
          ))}
        </Reveal3D>
      </section>
      ...
      <section className="bg-surface-2 py-24">
        <div className="mx-auto max-w-4xl px-6">
          <div className="mb-10 text-center">
            <TextReveal text="Ariza qoldiring" className="font-display text-4xl text-content sm:text-5xl" />
            <Reveal delay={0.15}><p className="mx-auto mt-4 max-w-lg text-muted">Zal va menyuni band qiling — biz siz bilan bog&apos;lanamiz.</p></Reveal>
          </div>
