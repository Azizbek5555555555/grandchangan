...
export default async function AboutPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <>
      ...
      <section className="overflow-hidden bg-surface-2 py-24 lg:py-32">
        <div className="mx-auto grid max-w-7xl items-center gap-16 px-6 lg:grid-cols-2">
          ...
          <div>
            <TextReveal text={st("aboutT1")} className="font-display text-4xl leading-tight text-content sm:text-5xl" />
            ...
            <RevealGroup className="mt-8 space-y-5 text-lg" stagger={0.15} y={22}>
              <p className="text-muted">{st("aboutP1")}</p>
              <p className="text-muted">{st("aboutP2")}</p>
            </RevealGroup>
            {g.address ? <Reveal delay={0.2}><p className="mt-8 text-sm text-muted">{st("aboutAddr")}: {t(g.address, locale)}</p></Reveal> : null}
          </div>
        </div>
      </section>
